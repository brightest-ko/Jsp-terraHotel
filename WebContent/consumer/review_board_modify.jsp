...
function button_check(){
	...
	<div class="container">
	<h2>Review 게시판</h2>
	<form action="ReviewBoardModifyAction.ro?pageno=<%=pageno%>" method="post" name="modifyform"enctype="multipart/form-data">
	<input type="hidden" name="rev_num" value="<%=board.getRev_num()%>">
		...
		<tr>
		<td>제 목</td> <td><input name="rev_subject" class="form-control"
					value="<%=board.getRev_subject()%>"> </td>
		</tr>
		<tr>
		<td>내 용</td>
				<td>
				<textarea name="rev_content"  rows="15" >
				<%=board.getRev_content().replace("\n","<br>")%></textarea>
				</td>				
			</tr>
			<tr>
		<td>
				...
				<!-- 파일첨부 이미지로 제한@@@ -->
				<td><input name="rev_file" type="file" accept="image/*" />&nbsp;첨부시 이전 파일이 삭제됩니다.</td>
			</tr>				
		<tr>
						<td>비밀번호</td>
						<td><input type="password"  class="form-control" name="rev_pw2" size="8">8자 이하 입력</td>
					</tr>		
		</table>			
	<input type="submit" name="modify"class="btn btn-default pull-right"  value="수 정" >
